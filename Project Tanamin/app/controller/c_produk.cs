...
namespace Project_Tanamin.app.controller
{
    public class c_produk
    {
        ...
        // Ambil semua produk
        public List<m_produk> GetProdukList()
        {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var p = new m_produk
                            {
                                ...
                                NamaKategori = reader["nama_kategori"] == DBNull.Value ? "" : reader.GetString(reader.GetOrdinal("nama_kategori")),
                                NamaProduk = reader["nama_produk"] == DBNull.Value ? "" : reader.GetString(reader.GetOrdinal("nama_produk")),
                                StokProduk = reader.GetInt32(reader.GetOrdinal("stok_produk")),
                                Deskripsi = reader["deskripsi"] == DBNull.Value ? "" : reader.GetString(reader.GetOrdinal("deskripsi")),
        ...
        // Tambah produk baru
        public bool AddProduk(m_produk p)
        {
            try
            {
                using (var conn = new NpgsqlConnection(db.connstring))
                {
                    ...
                    string query = @"
                        INSERT INTO produk 
                        (foto_produk, nama_kategori, nama_produk, stok_produk, deskripsi, harga_satuan, is_deleted) 
        ...
        // Update produk
        public bool UpdateProduk(m_produk p)
        {
            try
            {
                using (var conn = new NpgsqlConnection(db.connstring))
        ...
        // Hard delete
        public bool HardDeleteProduk(int id)
        {
            try
            {
                using (var conn = new NpgsqlConnection(db.connstring))
            ...
            {
                Console.WriteLine("Error HardDeleteProduk: " + ex.Message);
                return false;
            }
        }
    }
